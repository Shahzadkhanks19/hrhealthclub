...
export function FloatingActions() {
  ...
  return (
    <div className="fixed bottom-5 right-4 z-50 flex flex-col items-center gap-2.5 sm:right-5">
      <a href="#membership" className={`${actionClass} bg-[#7A0008] text-white hover:bg-[#450004] focus-visible:outline-[#7A0008]`} aria-label="Join HR Health Club">
        <Dumbbell size={20} />
        <span className={labelClass}>Join HR Health Club</span>
      </a>

      <a href="#free-trial" className={`${actionClass} bg-[#F8EEEE] text-[#7A0008] ring-1 ring-[#E2D6D6] hover:bg-white focus-visible:outline-[#7A0008]`} aria-label="Book a free trial">
        <CalendarCheck size={20} className="text-[#7A0008]" />
        <span className={labelClass}>Book Free Trial</span>
      </a>

      <a href="https://wa.me/918440070555?text=Hi%20HR%20Health%20Club%2C%20I%20would%20like%20to%20know%20more." target="_blank" rel="noreferrer" className={`${actionClass} bg-[#25D366] text-white hover:brightness-95 focus-visible:outline-[#25D366]`} aria-label="Chat with HR Health Club on WhatsApp">
        <FaWhatsapp size={22} />
