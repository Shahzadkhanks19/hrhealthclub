  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-[#E2D6D6] bg-white shadow-sm">
        <div className="mx-auto flex h-20 w-[min(1380px,calc(100%-2rem))] items-center justify-between gap-5">
          <Link href="/" aria-label="HR Health Club home" className="flex shrink-0 items-center transition duration-300 hover:scale-[1.025]"><Image src="/images/brand-logo.svg" width={167} height={48} alt="HR Health Club logo" className="h-12 w-auto object-contain" priority unoptimized /></Link>
          <nav className="hidden items-center gap-4 2xl:flex" aria-label="Primary navigation">{navItems.map(([label, href]) => { const active = isActive(href); return <Link key={label} href={href} aria-current={active ? "page" : undefined} className={`group relative py-2 font-[family-name:var(--font-display)] text-[13px] font-semibold tracking-wide transition duration-300 ${active ? "text-[#7A0008]" : "text-[#111111] hover:text-[#7A0008]"}`}><span>{label}</span><span className={`absolute inset-x-0 -bottom-0.5 h-0.5 origin-left rounded-full bg-[#7A0008] transition-transform duration-300 ${active ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} /></Link>; })}</nav>
          <Link href="/membership" className="hidden shrink-0 rounded-md bg-[#7A0008] px-5 py-3 font-[family-name:var(--font-display)] text-sm font-semibold text-white shadow-sm transition duration-300 hover:-translate-y-0.5 hover:bg-[#450004] hover:shadow-lg active:translate-y-0 lg:inline-flex">JOIN HR FAMILY</Link>
          <details className="relative 2xl:hidden"><summary className="flex cursor-pointer list-none items-center justify-center rounded-md border border-[#E2D6D6] p-2 text-[#7A0008] transition hover:bg-[#F8EEEE]" aria-label="Open menu"><Menu size={22} /></summary><nav className="absolute right-0 top-12 grid w-64 gap-1 rounded-xl border border-[#E2D6D6] bg-white p-3 shadow-xl" aria-label="Mobile navigation">{navItems.map(([label, href]) => { const active = isActive(href); return <Link key={label} href={href} aria-current={active ? "page" : undefined} className={`rounded-md px-3 py-2 font-[family-name:var(--font-display)] text-sm font-semibold transition ${active ? "bg-[#F8EEEE] text-[#7A0008]" : "hover:bg-[#F8EEEE] hover:text-[#7A0008]"}`}>{label}</Link>; })}<Link href="/membership" className="mt-2 rounded-md bg-[#7A0008] px-3 py-3 text-center font-[family-name:var(--font-display)] text-sm font-semibold text-white transition hover:bg-[#450004]">JOIN HR FAMILY</Link></nav></details>
        </div>
      </header>
    </>
  );
