...
const navItems = [["HOME", "#home"], ["ABOUT", "#about"], ["PROGRAMS", "#programs"], ["COACHES", "#coaches"], ["REELS", "#gallery"], ["GALLERY", "#gallery"], ["MEMBERSHIP", "#membership"], ["TESTIMONIALS", "#testimonials"], ["CONTACT", "#contact"]] as const;
      ...
      <div className="relative z-40 bg-[#7A0008] text-white">
        <div className="mx-auto flex min-h-11 w-[min(1280px,calc(100%-2rem))] items-center justify-between gap-6 text-xs lg:text-sm">
          <div className="hidden items-center gap-5 lg:flex">
            <span className="flex items-center gap-2"><MapPin size={16} />Jodhpur, Rajasthan</span><span className="h-5 w-px bg-white/35" />
            <span className="flex items-center gap-2"><Clock3 size={16} />Mon - Sat : 5:00 AM - 10:00 PM</span><span className="h-5 w-px bg-white/35" />
            <span className="flex items-center gap-2"><CalendarDays size={16} />Sunday : 6:00 AM - 2:00 PM</span>
          </div>
          <div className="ml-auto flex items-center gap-4">
            <a aria-label="Instagram" href="#gallery" className="transition hover:text-white/70"><FaInstagram size={17} /></a>
            <a aria-label="Facebook" href="#contact" className="transition hover:text-white/70"><FaFacebookF size={16} /></a>
            <a aria-label="YouTube" href="#gallery" className="transition hover:text-white/70"><FaYoutube size={18} /></a>
      ...
      <header className="sticky top-0 z-50 border-b border-[#E2D6D6] bg-white shadow-sm">
        <div className="mx-auto flex h-20 w-[min(1280px,calc(100%-2rem))] items-center justify-between gap-6">
          ...
          <nav className="hidden items-center gap-5 xl:flex" aria-label="Primary navigation">
            {navItems.map(([label, href], index) => <a key={label} href={href} className={`font-[family-name:var(--font-display)] text-sm font-semibold tracking-wide transition hover:text-[#7A0008] ${index === 0 ? "text-[#7A0008]" : "text-[#111111]"}`}>{label}</a>)}
          </nav>
